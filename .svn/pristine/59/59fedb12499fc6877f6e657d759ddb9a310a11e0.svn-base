...
for (var i = 0; i < allPic.length; i++) {
   ...
   $(allPic[i]).click(function(){
      ...
      console.log(top);
      showIframe(myPic,'800','800',top);
   });
}
//拉取搜索页面的iframe
function showIframe(url,w,h,t){
    //添加iframe
    var if_w = w; 
    var if_h = h; 
    //allowTransparency='true' 设置背景透明
    $("<iframe width='" + if_w + "' height='" + if_h + "' id='YuFrame1' name='YuFrame1' style='position:absolute;z-index:10;background-color:#fff;left:20%;top:20%;'  frameborder='no' marginheight='0' marginwidth='0' allowTransparency='true'></iframe>").prependTo('body');    
